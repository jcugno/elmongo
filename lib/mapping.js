...
exports.generateMapping = function (schema) {
  ...
  .forEach(function (path) {
    ...
    pathArray.forEach(function (pathEntry, i) {
      ...
      if (pathInfo.options && pathInfo.options.es_no_index ||
          pathInfo.caster && pathInfo.caster.options && pathInfo.caster.options.es_no_index) {

        delete currentLocation[pathEntry];
        return;
      }
      ...
      if (i === pathArray.length - 1) {
        ...
        if (pathInfo.options) {
          for (var prop in pathInfo.options) {
            // Map to field if it's an Elasticsearch option
            if (prop.indexOf('es_') === 0 && prop !== 'es_no_index') {
              currentLocation[pathEntry][prop.replace(/^es_/, '')] = pathInfo.options[prop];
            }
          }
        }

        if (pathInfo.caster && pathInfo.caster.options) {
          for (var prop in pathInfo.caster.options) {
            // Map to field if it's an Elasticsearch option
            if (prop.indexOf('es_') === 0 && prop !== 'es_no_index') {
              currentLocation[pathEntry][prop.replace(/^es_/, '')] = pathInfo.caster.options[prop];
            }
          }
        }
